...
export async function generateMetadata({
  ...
  return {
    ...
    description: cms?.excerpt || sector.lead,
  };
}
...
export default async function SetorPage({
  ...
  const heroImg = cmsPost?.coverUrl || r2Url(sector.img);
  const title = cmsPost?.title || `Segurança e adequação para ${sector.name}`;
  const lead = cmsPost?.excerpt || sector.lead;

  return (
    <div>
      <section className="relative overflow-hidden bg-ink-deep">
        <Image src={heroImg} alt="" fill sizes="100vw" priority className="object-cover opacity-30" />
        ...
        <div className="relative mx-auto max-w-[900px] px-7 py-16">
          ...
          <p className="mt-3 max-w-xl text-lg leading-relaxed text-white/85">{lead}</p>
        </div>
      </section>
